...
public class EPMetaTileEntityReinforcedRotorHolder extends MetaTileEntityMultiblockPart implements IMultiblockAbilityPart<IReinforcedRotorHolder>, IReinforcedRotorHolder {
    static final int SPEED_INCREMENT = 1;
    static final int SPEED_DECREMENT = 3;

    private final EPMetaTileEntityReinforcedRotorHolder.InventoryRotorHolder inventory;
    ...
    public EPMetaTileEntityReinforcedRotorHolder(ResourceLocation metaTileEntityId, int tier) {
        super(metaTileEntityId, tier);
        this.inventory = new EPMetaTileEntityReinforcedRotorHolder.InventoryRotorHolder();
        this.maxSpeed = 2000 + 1000 * tier;
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new EPMetaTileEntityReinforcedRotorHolder(metaTileEntityId, getTier());
    }
    ...
    public MultiblockAbility<IReinforcedRotorHolder> getAbility() {
        return EPMultiblockAbility.REINFORCED_ROTOR_MULTIBLOCK_ABILITY;
    }
    ...
    public void update() {
        ...
        if (controller != null && controller.isActive()) {
            ...
            if (getOffsetTimer() % 20 == 0) {
                damageRotor((int) ((1 + controller.getNumMaintenanceProblems()) * Math.pow(16, ((IMegaTurbine)getController()).getMode())));
            }
        } else if (!hasRotor()) {
            setCurrentSpeed(0);
    ...
    public void setRotor(ItemStack itemStack) {
        ...
        if (!getWorld().isRemote) {
            writeCustomData(EPDataCode.EP_CHANNEL_5, buf -> buf.writeInt(rotorColor));
            markDirty();
        }
    }
