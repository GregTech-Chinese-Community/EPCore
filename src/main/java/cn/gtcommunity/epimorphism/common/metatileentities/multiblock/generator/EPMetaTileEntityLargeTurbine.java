...
public class EPMetaTileEntityLargeTurbine extends FuelMultiblockController implements ITieredMetaTileEntity {
    ...
    public EPMetaTileEntityLargeTurbine(ResourceLocation metaTileEntityId, RecipeMap<?> recipeMap, int tier, IBlockState casingState, IBlockState gearboxState, ICubeRenderer casingRenderer, boolean hasMufflerHatch, ICubeRenderer frontOverlay) {
        ...
        this.tier = tier;
        this.recipeMapWorkable = new EPLargeTurbineWorkableHandler(this, tier);
        this.recipeMapWorkable.setMaximumOverclockVoltage(GTValues.V[tier]);
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new EPMetaTileEntityLargeTurbine(metaTileEntityId, recipeMap, tier, casingState, gearboxState, casingRenderer, hasMufflerHatch, frontOverlay);
    }
    ...
    protected void formStructure(PatternMatchContext context) {
        ...
        ((EPLargeTurbineWorkableHandler) this.recipeMapWorkable).updateTanks();
    }

    @Override
    protected long getMaxVoltage() {
        long maxProduction = ((EPLargeTurbineWorkableHandler) recipeMapWorkable).getMaxVoltage();
        long currentProduction = ((EPLargeTurbineWorkableHandler) recipeMapWorkable).boostProduction((int) maxProduction);
        if (isActive() && currentProduction < maxProduction) {
            return ((EPLargeTurbineWorkableHandler) recipeMapWorkable).getMaxVoltage();
        } else {
            return 0L;
        }
    }
    ...
    protected void addDisplayText(List<ITextComponent> textList) {
        ...
        if (isStructureFormed()) {
            ...
            if (rotorHolder.getRotorEfficiency() > 0) {
                ...
                long maxProduction = ((EPLargeTurbineWorkableHandler) recipeMapWorkable).getMaxVoltage();
                long currentProduction = isActive() ? ((EPLargeTurbineWorkableHandler) recipeMapWorkable).boostProduction((int) maxProduction) : 0;
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('R', metaTileEntities(MultiblockAbility.REGISTRY.get(EPMultiblockAbility.REINFORCED_ROTOR_MULTIBLOCK_ABILITY).stream()
