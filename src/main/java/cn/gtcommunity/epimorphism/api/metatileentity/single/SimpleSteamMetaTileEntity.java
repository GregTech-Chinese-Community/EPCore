...
public class SimpleSteamMetaTileEntity extends SteamMetaTileEntity {
    ...
    /**  SimpleSteamMetaTileEntity(ResourceLocation metaTileEntityId, RecipeMap<?> recipeMap, SteamProgressIndicator progressIndicator, ICubeRenderer renderer, boolean isBrickedCasing, boolean isHighPressure)
    ...
    protected TextureArea getCircuitSlotOverlay() {
        return EPGuiTextures.INT_CIRCUIT_OVERLAY_STEAM.get(isHighPressure);
    }
    ...
    //  Bronze slot for steam, steel slot for high press steam.
    protected TextureArea[] getOverlaysForSlot(boolean isOutputs, boolean isFluid) {
        return new TextureArea[] {isFluid ? EPGuiTextures.FLUID_SLOT_STEAM.get(isHighPressure) : GuiTextures.SLOT_STEAM.get(isHighPressure)};
    }
