...
//        if (EPAPI.MAP_GLASS.containsKey(blockState)) {
//            ITier stats = EPAPI.MAP_GLASS.get(blockState);
...
//        }
//        return false;
//    }, () -> EPAPI.MAP_GLASS.entrySet().stream()
//            // sort to make autogenerated jei previews not pick random glasses each game load
//            .sorted(Comparator.comparingInt(entry -> (int) entry.getValue().getTire()))
//            .map(entry -> new BlockInfo(entry.getKey(), null))
//            .toArray(BlockInfo[]::new))
//            .addTooltips("epimorphism.multiblock.pattern.error.glasses");
//    public static Supplier<TraceabilityPredicate> EP_OPTICAL_GLASS = () -> new TraceabilityPredicate(blockWorldState -> {
//        IBlockState blockState = blockWorldState.getBlockState();
//        if (EPAPI.MAP_GLASS.containsKey(blockState)) {
//            ITier stats = EPAPI.MAP_GLASS.get(blockState);
//            if (((ITierGlassBlockState)stats).isOpticalGlass()) {
//                Object currentGlass = blockWorldState.getMatchContext().getOrPut("CasingType", stats);
//                if (!currentGlass.equals(stats)) {
//                    blockWorldState.setError(new PatternStringError("epimorphism.multiblock.pattern.error.glasses"));
//                    return false;
...
//        return false;
//    }, () -> EPAPI.MAP_GLASS.entrySet().stream()
//            .filter(entry -> ((ITierGlassBlockState)entry.getValue()).isOpticalGlass())
//            // sort to make autogenerated jei previews not pick random glasses each game load
//            .sorted(Comparator.comparingInt(entry -> (int) entry.getValue().getTire()))
//            .map(entry -> new BlockInfo(entry.getKey(), null))
//            .toArray(BlockInfo[]::new))
//            .addTooltips("epimorphism.multiblock.pattern.error.glasses");
//    public static Supplier<TraceabilityPredicate> EP_GLASS_NO_OPTICAL = () -> new TraceabilityPredicate(blockWorldState -> {
//        IBlockState blockState = blockWorldState.getBlockState();
//        if (EPAPI.MAP_GLASS.containsKey(blockState)) {
//            ITier stats = EPAPI.MAP_GLASS.get(blockState);
//            if (!((ITierGlassBlockState)stats).isOpticalGlass()) {
